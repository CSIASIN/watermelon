<?php

/**
 * Breadcrumb
 *
 * @package Watermelon_Wordpress_Theme
 * @version 6.1.0
 */


// Exit if directly accessed
defined('ABSPATH') || exit;


/**
 * Breadcrumb
 */
if (!function_exists('wm_breadcrumb')) :
  function wm_breadcrumb() {

    if (!is_home()) {
      echo '<nav aria-label="breadcrumb" class="overflow-x-auto text-nowrap mb-4 mt-2 py-2 px-3 bg-body-tertiary rounded">';
      echo '<ol class="breadcrumb flex-nowrap mb-0">';
      echo '<li class="breadcrumb-item"><a href="' . HOMEURL . '">' . '' . '<svg class="bi ms-auto" aria-hidden="true"><use href="#bi-house-door-fill"></use></svg>'. '<span class="visually-hidden">' . __('Home', 'bootscore') . '</span>' . '</a></li>';
      // display parent category names with links
      if (is_category() || is_single()) {
        $cat_IDs = wp_get_post_categories(get_the_ID());
        foreach ($cat_IDs as $cat_ID) {
          $cat = get_category($cat_ID);
         echo '<li class="breadcrumb-item"><a class="' . apply_filters('bootscore/class/breadcrumb/item/link', '') . '" href="' . get_term_link($cat->term_id) . '">' . $cat->name . '</a></li>';
        }
      }
      // display current page name
      if (is_page() || is_single()) {
        echo '<li class="breadcrumb-item active" aria-current="page">' . get_the_title() . '</li>';
      }
      echo '</ol>';
      echo '</nav>';
    }
  }

  add_filter('breadcrumbs', 'wm_breadcrumb');
endif;
